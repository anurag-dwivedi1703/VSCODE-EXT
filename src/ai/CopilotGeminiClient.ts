...
export class CopilotGeminiClient {
    ...
    public startSession(systemPrompt: string, _thinkingLevel: 'low' | 'high' = 'high', includeToolInstructions: boolean = true, tools?: vscode.LanguageModelChatTool[]): ISession {
        ...
        console.log(`[CopilotGeminiClient] Session mode: ${useNativeTools ? 'NATIVE tool calling' : includeToolInstructions ? 'LEGACY text-parsed tool calling' : 'NO tools (refinement)'} (${tools?.length ?? 0} tools provided)`);
        ...
        console.log(`[CopilotGeminiClient] Context pruning: ${FEATURE_FLAGS.ENABLE_CONTEXT_PRUNING ? 'enabled' : 'disabled'} (ENABLE_CONTEXT_PRUNING flag)`);
        ...
        function pruneMessagesIfNeeded(): void {
            ...
            console.log(
                `[CopilotGeminiClient] \u{1F504} Context pruned: ${beforeCount} \u{2192} ${messages.length} messages. ` +
        ...
        return {
            sendMessage: async (prompt: string | any[]) => {
                if (!model) {
                    return {
                        response: {
                            text: () => 'Error: Copilot Gemini model not initialized. Ensure GitHub Copilot is installed and you have an active subscription.',
                ...
                try {
                        ...
                        if (toolResponses.length > 0 && useNativeTools && lastToolCallParts.length > 0) {
                            ...
                            const resultParts: (vscode.LanguageModelToolResultPart)[] = toolResponses.map((tr: any, index: number) => {
                                ...
                                if (index >= lastToolCallParts.length) {
                                    console.warn(`[CopilotGeminiClient] Tool result index ${index} exceeds lastToolCallParts length ${lastToolCallParts.length}, using last call ID`);
                                }
                            ...
                            console.log(`[CopilotGeminiClient] Native tool results: ${toolResponses.length} results paired with ${lastToolCallParts.length} call(s)`);
                    ...
                    for (let attempt = 0; attempt < MAX_API_RETRIES; attempt++) {
                        try {
                            ...
                            const isTransient = CopilotGeminiClient.isTransientError(retryErr);

                            if (isTransient && attempt < MAX_API_RETRIES - 1) {
                                const waitMs = RETRY_BACKOFF_MS[attempt];
                                console.warn(
                                    `[CopilotGeminiClient] ⚠️ RETRY ${attempt + 1}/${MAX_API_RETRIES} - Transient error: "${retryErr.message}". ` +
                            ...
                            console.error(
                                `[CopilotGeminiClient] ❌ API call failed after ${attempt + 1} attempt(s): "${retryErr.message}" ` +
                    ...
                    if (lastApiError) {
                        ...
                        console.error(
                            `[CopilotGeminiClient] 🛑 All ${MAX_API_RETRIES} retries exhausted. Context cleaned up ` +
                    ...
                    if (useNativeTools) {
                        ...
                        if (functionCalls.length > 0) {
                            console.log(`[CopilotGeminiClient] Native tool calls: ${functionCalls.map(c => c.name).join(', ')}`);
                        }
                    ...
                    console.error(`[CopilotGeminiClient] API Error (after retries): ${error.message}`);
                    ...
                    const isTransient = CopilotGeminiClient.isTransientError(error);
