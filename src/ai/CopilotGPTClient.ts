...
` : '';  // End of toolCallInstructions ternary
    ...
    private parseTextDiffs(text: string): { name: string; args: any }[] {
        ...
        while ((match = diffBlockPattern.exec(text)) !== null) {
            ...
            if (filePath && diffContent) {
                ...
                console.log(`[CopilotGPTClient] Parsed text-based apply_diff for: ${filePath}`);
            }
        }

        if (calls.length > 0) {
            console.log(`[CopilotGPTClient] Found ${calls.length} text-based diff blocks`);
        }
    ...
    private detectTruncation(text: string): boolean {
        ...
        if (trimmed.includes('[APPLY_DIFF:') && !trimmed.includes('[END_DIFF]')) {
            console.log('[CopilotGPTClient] Truncation: Incomplete [APPLY_DIFF:] block');
            return true;
        }
